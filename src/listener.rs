...
use tokio::{
    io::{BufReader, BufWriter, stderr, stdin, stdout},
    net::TcpListener,
    sync::mpsc,
};
    ...
    match listener.accept().await {
        Ok(s) => {
            ...
            let stdin_handle = tokio::spawn(async move {
                ...
                loop {
                    ...
                    let n = bufreader
                        .read_line(&mut line)
                        .await
                        .expect("Failed to read line from stdin (listener)");
                    ...
                    socket_stdin
                        ...
                        .await
                        .context("Failed to write stdin to socket")
                        .expect("Failed to write stdin to socket");
                    socket_stdin
                        .flush()
                        .await
            ...
            let stdout_handle = tokio::spawn(async move {
                ...
                tokio::io::copy(&mut socket_stdout, &mut stdout)
                    .await
                    .expect("Failed to copy from the socket to stdout");
                tokio::io::copy(&mut socket_stdout, &mut stderr)
                    .await
                    .expect("Failed to copy from the socket to stderr");
            });
